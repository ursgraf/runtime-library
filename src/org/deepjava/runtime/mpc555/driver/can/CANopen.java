...
package org.deepjava.runtime.mpc555.driver.can;
...
//	private static final short sync_COB = 0x080;	// COB-ID for synch signal
	...
	public static void sendSDO(byte id, short index, byte subIndex, int val, int len) {
		CANA.setMsgBufRx1(0, sc_SDO_COB | id, false, data);	// set receive msg buffer to search for SDO answer
		switch (len) {
		case 4:
		...
		CANA.setMsgBufTx(len, cs_SDO_COB | id, false, data);	
		CANA.waitForTransferComplete(CANA.rx1BufNo);	// wait for answer
	}
	...
	public static void sendMsg0NMTenterPreOp(int id) {
		data[0] = (byte)0x80;
		data[1] = (byte)id;
		CANA.setMsgBufTx(2, 0, false, data);	
		CANA.waitForTransferComplete(CANA.txBufNo);	// wait for end of transfer
	}
	...
	public static void start(int id) {	
		CANA.setMsgBufRx1(0, bootUpMsg | id, false, data);	// set receive msg buffer to listen for bootup msg
		sendMsg0NMTresetNode(id);	// reset all nodes
		CANA.waitForTransferComplete(CANA.rx1BufNo);	// wait for end of bootup msg
	}
	
	public static void dispMsgBuf1() {
		System.out.print("Message Buffer 1");
		byte data[] = CANA.getMsgBuf(1);
		int len = data[1] &  0xf;
	...
	public static void printSDOAnswer() {
		data = CANA.getMsgBuf(CANA.rx1BufNo);
	...
	public static void sendSync() {
		CANA.setMsgBufTx(0, 0x80, false, data);
	}

	public static void setMsgBufRxPDO(int id) {
		CANA.setMsgBufRx2(0, PDO1_COB + id, false, data);
