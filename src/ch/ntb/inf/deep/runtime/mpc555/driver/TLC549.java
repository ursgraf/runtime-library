...
public class TLC549 {
	...
	public static void init() {
		...
		US.PUT1(QSMCM.PQSPAR, 0xB); 	//use PCS0, MOSI, MISO for QSPI //
		US.PUT1(QSMCM.DDRQS, 0xE); 	//SCK, MOSI, PCS0 outputs; MISO is input
		US.PUT2(QSMCM.PORTQS, 0xFF); 	//all Pins, in case QSPI disabled, are high 
		US.PUT2(QSMCM.SPCR0, 0xA028); 	//QSPI is master, 8 bits per transfer, inactive state of 
															//SCLK is LOW (CPOL=0), data captured on leading edge (CPHA=0), clock = 0.5 MHz 
		US.PUT2(QSMCM.SPCR2, 0x4000); 	// no interrupts, wraparound mode, NEWQP=0, ENDQP=0
		US.PUT1(QSMCM.CR, 0x3E); 	//disable chip select after transfer, 8 bits,  DT=1, DSCK=1, use PCS0 
		US.PUT2(QSMCM.SPCR1, 0xB816);	// enable QSPI, 1.4usec delay from PCS to SCK,
