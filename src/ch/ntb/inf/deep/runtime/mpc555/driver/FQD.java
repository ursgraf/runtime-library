...
public class FQD {
	...
	public static void init(boolean tpuA, int channel) {
		 if(tpuA){
			 ...
			 short s = SYS.GET2(TPU_A.CFSR3 -(channel / 4) * 2);
			 s &= ~(0xFF << shiftl);
			 s |= (0x66 << shiftl);
			 SYS.PUT2(TPU_A.CFSR3 - (channel / 4) * 2, s);
			
			 //ch14: Position_Count = 0
			 SYS.PUT2(TPU_A.TPURAM0 + 0x10 * channel + 2, 0);
			 //Edge_time_LSB_Addr = TPRAM14+1
			 SYS.PUT2(TPU_A.TPURAM0 + 0x10 * channel +10, TPU_A.TPURAM0  + 0x10 * channel + 1);
			 //Edge_Time_LSB_addr = TPRAM14 + 1
			 SYS.PUT2(TPU_A.TPURAM0 + 0x10 * (channel + 1) + 10, TPU_A.TPURAM0 +0x10 * channel  + 1);
			 //Corr_Pinstate_addr = TPRAM15+6
			 SYS.PUT2(TPU_A.TPURAM0 + 0x10 * channel + 8, TPU_A.TPURAM0 +0x10 * (channel + 1) + 6);
			 //Corr_Pinstate_addr = TPRAM14+6
			 SYS.PUT2(TPU_A.TPURAM0 + 0x10 * (channel + 1) + 8, TPU_A.TPURAM0 +0x10 *channel  +6);
			 
			 shiftl = (channel % 8) * 2;
			 //Channel is primary, ch+ 1 is secondary channel
			 s = SYS.GET2(TPU_A.HSQR1 - (channel / 8) * 2);
			 s &= ~(0x9 << shiftl);
			 s |= (0x4 << shiftl);
			 SYS.PUT2(TPU_A.HSQR1 - (channel / 8) * 2,s);
			
			 //Initalize channel and ch + 1	 
			 s = SYS.GET2(TPU_A.HSRR1 - (channel / 8) * 2);
			 s &= ~(0x9 << shiftl);
			 s |= (0xF << shiftl);
			 SYS.PUT2(TPU_A.HSRR1 - (channel / 8) * 2,s);
			
			 //set priority high
			 s = SYS.GET2(TPU_A.CPR1 - (channel / 8) * 2);
			 s &= ~(0x9 << shiftl);
			 s |= (0xF << shiftl);
			 SYS.PUT2(TPU_A.CPR1 - (channel / 8) * 2, s);
		 }else{
			 int shiftl = (channel % 4) * 4;
			 //initalize TPU for quadrature decode function code = 6;
			 short s = SYS.GET2(TPU_B.CFSR3 -(channel / 4) * 2);
			 s &= ~(0xFF << shiftl);
			 s |= (0x66 << shiftl);
			 SYS.PUT2(TPU_B.CFSR3 - (channel / 4) * 2, s);
			 
			 //ch14: Position_Count = 0
			 SYS.PUT2(TPU_B.TPURAM0 + 0x10 * channel + 2, 0);
			 //Edge_time_LSB_Addr = TPRAM14+1
			 SYS.PUT2(TPU_B.TPURAM0 + 0x10 * channel +10, TPU_B.TPURAM0  + 0x10 * channel + 1);
			 //Edge_Time_LSB_addr = TPRAM14 + 1
			 SYS.PUT2(TPU_B.TPURAM0 + 0x10 * (channel + 1) + 10, TPU_B.TPURAM0 +0x10 * channel  + 1);
			 //Corr_Pinstate_addr = TPRAM15+6
			 SYS.PUT2(TPU_B.TPURAM0 + 0x10 * channel + 8, TPU_B.TPURAM0 +0x10 * (channel + 1) + 6);
			 //Corr_Pinstate_addr = TPRAM14+6
			 SYS.PUT2(TPU_B.TPURAM0 + 0x10 * (channel + 1) + 8, TPU_B.TPURAM0 +0x10 *channel  +6);
			 
			 shiftl = (channel % 8) * 2;
			 //Channel is primary, ch+ 1 is secondary channel
			 s = SYS.GET2(TPU_B.HSQR1 - (channel / 8) * 2);
			 s &= ~(0x9 << shiftl);
			 s |= (0x4 << shiftl);
			 SYS.PUT2(TPU_B.HSQR1 - (channel / 8) * 2,s);
			 
			 //Initalize channel and ch + 1	 
			 s = SYS.GET2(TPU_B.HSRR1 - (channel / 8) * 2);
			 s &= ~(0x9 << shiftl);
			 s |= (0xF << shiftl);
			 SYS.PUT2(TPU_B.HSRR1 - (channel / 8) * 2,s);
			 
			 //set priority high
			 s = SYS.GET2(TPU_B.CPR1 - (channel / 8) * 2);
			 s &= ~(0x9 << shiftl);
			 s |= (0xF << shiftl);
			 SYS.PUT2(TPU_B.CPR1 - (channel / 8) * 2, s);
	...
	public static short getPosition(boolean tpuA, int channel) {
		if(tpuA){
			return SYS.GET2(TPU_A.TPURAM0 + 0x10 * channel + 2);
		}else{
			return SYS.GET2(TPU_B.TPURAM0 + 0x10 * channel + 2);
		}
	}
	...
	public static void setPosition(boolean tpuA, int channel, int pos) {
		if(tpuA){
			SYS.PUT2(TPU_A.TPURAM0 + 0x10 * channel + 2, pos);
		}else{			
			SYS.PUT2(TPU_B.TPURAM0 + 0x10 * channel + 2, pos);
		}
	}
