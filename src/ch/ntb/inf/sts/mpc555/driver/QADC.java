...
public class QADC {
	...
	public static short read(boolean qadcA, int channel) {
		...
		if (qadcA) {
			return SYS.GET2(RJURR_A + channelOffset + ADDR_OFFSET);
		} else {
			return SYS.GET2(RJURR_B + channelOffset + ADDR_OFFSET);
		}
	}
	...
	public static void init(boolean qadcA) {
		if (qadcA) {
			...
			SYS.PUT2(QACR2_A, 0x1890);

			// CCW for AN0 - AN3, max sample time
			// ADDR_OFFSET: Using queue2
			for (int i = 0; i <= 3; i++) {
				int addr = i * 2;
				SYS.PUT2(CCW_A + ADDR_OFFSET + addr, CCW_INIT + i);
			}
			...
			// end of queue
			SYS.PUT2(CCW_A + ADDR_OFFSET + 16 * 2, END_OF_QUEUE);
		} else {
			// user access
			SYS.PUT2(QADCMCR_B, 0);
			
			// internal multiplexing, use ETRIG1 for queue1, QCLK = 40 MHz / (11+1 + 7+1) = 2 MHz
			SYS.PUT2(QACR0_B, 0x00B7);
			...
			// queue2 begins at CCW + 2*16 (32 = ADDR_OFFSET)
			// This offset is used because of the DistSense driver
			SYS.PUT2(QACR2_B, 0x1890);

			// CCW for AN0 - AN3, max sample time
			// ADDR_OFFSET: Using queue2
			for (int i = 0; i <= 3; i++) {
				int addr = i * 2;
				SYS.PUT2(CCW_B + ADDR_OFFSET + addr, CCW_INIT + i);
			}
			...
			// end of queue
			SYS.PUT2(CCW_B + ADDR_OFFSET + 16 * 2, END_OF_QUEUE);
		}
