...
public class DIO {
	...
	public static void init(boolean tpuA, int channel, boolean out) {
		if(tpuA){
			...
			if(out){
				SYS.PUT2(TPU_A.TPURAM0 + 0x10 * channel, 0x3);
			}else{
				s = SYS.GET2(TPU_A.HSQR1 - (channel / 8) * 2);
				s &= ~(3 << shiftl);
				SYS.PUT2(TPU_A.HSQR1 -(channel / 8) * 2, s);
				SYS.PUT2(TPU_A.TPURAM0 + 0x10 * channel, 0xF);
			}
			...
			if(out){
				SYS.PUT2(TPU_B.TPURAM0 + 0x10 * channel, 0x3);
			}else{
				s = SYS.GET2(TPU_B.HSQR1 - (channel / 8) * 2);
				s &= ~(3 << shiftl);
				SYS.PUT2(TPU_B.HSQR1 -(channel / 8) * 2, s);
				SYS.PUT2(TPU_B.TPURAM0 + 0x10 * channel, 0xF);
			}
	...
	public static boolean in(boolean tpuA, int channel) {
		if(tpuA){
			return (SYS.GET2(TPU_A.TPURAM0 + 0x10 * channel + 2) & (1 << 15)) != 0; 
		}else{
			return (SYS.GET2(TPU_B.TPURAM0  + 0x10 * channel + 2) & (1 << 15)) != 0; 
		}
	}
	...
	public static void out(boolean tpuA, int channel, boolean val) {
		if(tpuA){
			...
			short sh = SYS.GET2(TPU_A.CISR);
			SYS.PUT2(TPU_A.CISR,(short)0);
			
			short s = SYS.GET2(TPU_A.HSRR1 - ((channel / 8) * 2));
			...
			//Restore Interrupts
			SYS.PUT2(TPU_A.CISR, sh);
		}else{
			//Disable all Interrupts
			short sh = SYS.GET2(TPU_B.CISR);
			SYS.PUT2(TPU_B.CISR,(short)0);
			
			int shiftl = (channel % 8) * 2;
			short s = SYS.GET2(TPU_B.HSRR1 - ((channel / 8) * 2));
			s &= ~(3 << shiftl);
			if(val) s |= (1 << shiftl);
			else s |= (2 << shiftl);
			SYS.PUT2(TPU_B.HSRR1 - ((channel / 8) * 2), s);
